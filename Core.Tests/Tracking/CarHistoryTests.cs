...
namespace LMUSessionTracker.Core.Tests.Tracking {
	public class CarHistoryTests {
		...
		public void Update_NoLapsCompleted_NoLaps() {
			Lap ac = history.Update(new() { lapsCompleted = 0 }, dt);
		...
		public void Update_OneInvalidLapCompleted_OneLap() {
			...
			Lap ac1 = history.Update(new() { lapsCompleted = 1, lastSectorTime1 = -1, lastSectorTime2 = -1, lastLapTime = -1 }, dt);
		...
		public void Update_OneValidLapCompleted_OneLap() {
			...
			Lap ac1 = history.Update(new() { lapsCompleted = 1, lastSectorTime1 = 10, lastSectorTime2 = 30, lastLapTime = 60 }, dt);
			Assert.Equal(1, history.LapsCompleted);
			AssertLap(ex1, ac1);
			Assert.Equal(ac1, history.GetLap(1));
		}
		...
		public void Update_LapSkipped_TwoLaps() {
			...
			Lap ac2 = history.Update(new() { lapsCompleted = 2, lastSectorTime1 = 10, lastSectorTime2 = 30, lastLapTime = 60 }, dt);
		...
		public void Update_AddOutOfOrder_TwoLaps() {
			...
			Lap ex2 = new Lap() { LapNumber = 2, Sector1 = 10, Sector2 = 20, Sector3 = 30, TotalTime = 60 };
			Lap ac2 = history.Update(new() { lapsCompleted = 2, lastSectorTime1 = 10, lastSectorTime2 = 30, lastLapTime = 60 }, dt);
			Lap ac1 = history.Update(new() { lapsCompleted = 1, lastSectorTime1 = 11, lastSectorTime2 = 33, lastLapTime = 66 }, dt);
			Assert.Equal(2, history.LapsCompleted);
			AssertLap(ex1, ac1);
			AssertLap(ex2, ac2);
			Assert.Equal(ac1, history.GetLap(1));
