...
namespace LMUSessionTracker.Common.Tests.Services {
	public class ClientServiceTests {
		...
		private readonly Mock<ClientSessionState> sessionState;
		...
		public ClientServiceTests(LoggingFixture loggingFixture) {
			...
			sessionState = new Mock<ClientSessionState>();
			...
			handlerFactory.Setup(x => x.Create(It.IsAny<LMUClient>(), It.IsAny<ProtocolClient>(), It.IsAny<ClientInfo>(), It.IsAny<ClientIntervalProvider>(), It.IsAny<ClientSessionState>())).Returns(handler.Object);
			...
			scope.Setup(x => x.ServiceProvider.GetService(typeof(ClientSessionState))).Returns(sessionState.Object);
